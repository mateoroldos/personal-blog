...
const getBlogUniqueTags = (posts: CollectionEntry<'blog'>[]) => {
  const tags: string[] = posts
    ...
    .filter((value, index, self) => self.findIndex((tag) => tag === value) === index)
...
const getUniqueTags = (posts: CollectionEntry<'blog'>[], projects: Project[]) => {
  const projectTags = getProjectUniqueTags(projects);
  const blogTags = getBlogUniqueTags(posts);

  const tags = [...projectTags, ...blogTags].filter((value, index, self) => self.indexOf(value) === index);
